...
import { useGetAcupuntureQuery } from "../../../redux/acupunctures/acupuncturesApi";
...
function AcupunctureDetail(props) {
  const { id } = useParams();
  ...
  const TABS = [
    ...
    { name: "Create Topic", component: CreatePostEditor, props: {id, createdAt: "Acupuncture"} },
  ];

  const [activeTab, setActiveTab] = useState(TABS[0].name);
  const { data: acupuncture, isLoading, isError } = useGetAcupuntureQuery(id);
    ...
    content = (
      <>
        <Box sx={{ marginBottom: "30px" }}>
          <Typography variant="h4">{acupuncture.name}</Typography>
        ...
        >
          <Box>
            ...
            <ul>
              <ItemList listName="Pinyin" value={acupuncture?.pinyin} />
              <ItemList listName="English" value={acupuncture?.english} />
              <ItemList listName="Japanese" value={acupuncture?.japanese} />
              <ItemList listName="Korean" value={acupuncture?.korean} />
              <ItemList listName="Vietnamese" value={acupuncture?.vietnamese} />
            </ul>
          </Box>
          <Box>
...
export default AcupunctureDetail;
