...
import { useGetMateriaMedicasQuery } from "../../redux/materiaMedica/materiaMedicaApi";
...
const MateriaMedicaList = () => {
  ...
  const applyFilter = (data) => {
    ...
    return data.filter(
      (item) =>
        ...
        item.name?.includes(character.toUpperCase())
    );
  };
  ...
  if (!isLoading && !isError && materiamedicas.length > 0) {
    const filteredData = applyFilter(materiamedicas);
