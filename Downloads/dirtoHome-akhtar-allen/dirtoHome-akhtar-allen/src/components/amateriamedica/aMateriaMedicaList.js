...
const aMateriaMedicaList = () => {
  const { data: amateriamedica, isLoading, isError } = useGetaMateriaMedicaQuery();
  ...
  // main content ui
  if (!isLoading && !isError && amateriamedica.length > 0) {
    const filteredData = applyFilter(amateriamedica);
  ...
  if (!isLoading && !isError && materiamedicas.length === 0) {
...
export default aMateriaMedicaList;
